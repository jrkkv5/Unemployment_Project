...
The focus of this report is to examine unemployment within the United States.  With unemployment as the dependent variable we examine its relationship with the house price index (HPI), real median household income (RMHI), poverty rate, population, and change in the S&P 500 index. The unemployment rates encompassed within our dataset are measured at the state level. In the ensuing report we provide a brief overview of all of the observations in the dataset and move on to examine two states, Illinois and West Virginia, at a more detailed level. This report seeks to prove that house price index (HPI), real median household income (RMHI), poverty rate, population, and change in the S&P 500 index have no direct effect on the unemployment rate at the state level.   
...
In the above code section the data is imported into R Studio and stored in the dataframe named merged_final.

As a final step before analyzing the data, two new variables are created to provide the logarithmic values of population and RMHI. By taking the logarithmic values, these variables become more comparable to the existing variables we have within our dataset. Below you can see the table output once all of our data has been merged.
...
stat.desc(c(merged_final$unemployment,merged_final$poverty))
...
In conclusion, our null hypothesis did not hold up against the data sample.  While there did not appear to be a significant relationship between the independent variables and unemployment rate at the state level, the explanatory variables did appear to have some effect on the unemployment rate. In this case we reject the null hypothesis as the other variables did appear to have an effect on the response variable, unemployment.  Further data is needed in order to improve the dataset and explore the null hypothesis further. Additional variables that could improve the model include the following demographics: sex, age, ethnicity, and education level. 