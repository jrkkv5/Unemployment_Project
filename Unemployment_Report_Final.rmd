...
After all of the data was merged we binned the states into groups based on the mean population of the years in our dataframe (1984-2018).  Below are the bins that were created:
...
IL_reg1 <- lm(unemployment ~ poverty + RMHI + HPI + population + sp500, data = IL_data)
summary(IL_reg1)
anova(IL_reg1)
```

```{r IL linear regression log all}
IL_reg2 <- lm(unemployment ~ poverty + log_RMHI + HPI + log_pop + sp500, data = IL_data)
summary(IL_reg2)
anova(IL_reg2)
```

```{r IL linear significant variables}
IL_linearModelSignificant <- lm(unemployment ~ poverty + log_pop, data = IL_data)
...
#run multiple linear model for data
WV_reg1 <- lm(unemployment ~ poverty + RMHI + HPI + population + sp500, data = WV_data)
summary(WV_reg1)
anova(WV_reg1)
```

```{r WV linear regression log all}
WV_reg2 <- lm(unemployment ~ poverty + log_RMHI + HPI + log_pop + sp500, data = WV_data)
summary(WV_reg2)
anova(WV_reg2)
```

```{r WV linear significant variables}
WV_linearModelSignificant <- lm(unemployment ~ poverty + log_pop, data = WV_data)
